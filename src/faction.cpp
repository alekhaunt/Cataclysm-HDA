...
int npc::faction_display( const catacurses::window &fac_w, const int width ) const
{
    ...
    const std::pair <std::string, nc_color> arousal_pair = display::arousal_text_color( *this );
    ...
    mvwprintz(fac_w, point(width, ++y), arousal_pair.second,
               _( "Arousal: " ) + ( arousal_pair.first.empty() ? nominal : arousal_pair.first ) );
